...
export default function AddSelfOp({typeInfo, keyState, keyTypeInfo, resultRows}: {typeInfo: StoredTypeInfo, keyState: string, keyTypeInfo: StoredTypeInfo, resultRows: number})
{

    const label = `${typeInfo.lcTypeName}AddSelfOp`

    const mutation = `mutation AddSelfOp($key: ${keyTypeInfo.gqlTypeName}!) {
        ${label}(key: $key)
    }`
