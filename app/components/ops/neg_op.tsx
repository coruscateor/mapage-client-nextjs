...
export default function NegOp({typeInfo, keyState, keyTypeInfo, resultRows}: {typeInfo: StoredTypeInfo, keyState: string, keyTypeInfo: StoredTypeInfo, resultRows: number})
{

    const label = `${typeInfo.lcTypeName}NegOp`

    const mutation = `mutation MulOp($key: ${keyTypeInfo.gqlTypeName}!) {
        ${label}(key: $key)
    }`
