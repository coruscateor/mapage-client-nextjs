...
export default function DivSelfOp({typeInfo, keyState, keyTypeInfo, resultRows}: {typeInfo: StoredTypeInfo, keyState: string, keyTypeInfo: StoredTypeInfo, resultRows: number})
{

    const label = `${typeInfo.lcTypeName}DivSelfOp`

    const mutation = `mutation DivSelfOp($key: ${keyTypeInfo.gqlTypeName}!) {
        ${label}(key: $key)
    }`
