...
export default function IncOp({typeInfo, keyState, keyTypeInfo, resultRows}: {typeInfo: StoredTypeInfo, keyState: string, keyTypeInfo: StoredTypeInfo, resultRows: number})
{

    const label = `${typeInfo.lcTypeName}IncOp`

    const mutation = `mutation IncOp($key: ${keyTypeInfo.gqlTypeName}!) {
        ${label}(key: $key)
    }`
