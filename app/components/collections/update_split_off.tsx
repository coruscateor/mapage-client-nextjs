...
export default function UpdateSplitOff({typeInfo, keyState, keyTypeInfo, resultRows}: {typeInfo: StoredTypeInfo, keyState: string, keyTypeInfo: StoredTypeInfo, resultRows: number})
{

    const label = `${typeInfo.lcTypeName}UpdateSplitOff`

    const mutation = `mutation UpdateSplitOfft($key: ${keyTypeInfo.gqlTypeName}!, $at: Int!) {
        ${label}(key: $key, at: $at)
    }`
    
    const paramComps = new ParamComps(label, new ParamComp("at", 3, JsConversionType.Number))
