...
export default function ReadLast({typeInfo, keyState, keyTypeInfo, resultRows}: {typeInfo: StoredTypeInfo, keyState: string, keyTypeInfo: StoredTypeInfo, resultRows: number})
{

    const label = `${typeInfo.lcTypeName}ReadLast`

    const query = `query ReadLast($key: ${keyTypeInfo.gqlTypeName}!) {
        ${label}(key: $key)
    }`
