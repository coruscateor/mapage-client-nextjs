...
export default function UpdateSortUnstable({typeInfo, keyState, keyTypeInfo, resultRows}: {typeInfo: StoredTypeInfo, keyState: string, keyTypeInfo: StoredTypeInfo, resultRows: number})
{

    const label = `${typeInfo.lcTypeName}UpdateSortUnstable`

    const mutation = `mutation UpdateSortUnstable($key: ${keyTypeInfo.gqlTypeName}!) {
        ${label}(key: $key)
    }`
