...
export default function UpdateDedup({typeInfo, keyState, keyTypeInfo, resultRows}: {typeInfo: StoredTypeInfo, keyState: string, keyTypeInfo: StoredTypeInfo, resultRows: number})
{

    const label = `${typeInfo.lcTypeName}UpdateDedup`

    const mutation = `mutation UpdateDedup($key: ${keyTypeInfo.gqlTypeName}!) {
        ${label}(key: $key)
    }`
