...
export default function ReadFirst({typeInfo, keyState, keyTypeInfo, resultRows}: {typeInfo: StoredTypeInfo, keyState: string, keyTypeInfo: StoredTypeInfo, resultRows: number})
{

    const label = `${typeInfo.lcTypeName}ReadFirst`

    const query = `query ReadFirst($key: ${keyTypeInfo.gqlTypeName}!) {
        ${label}(key: $key)
    }`
