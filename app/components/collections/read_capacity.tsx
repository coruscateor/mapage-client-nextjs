...
export default function ReadCapacity({typeInfo, keyState, keyTypeInfo, resultRows}: {typeInfo: StoredTypeInfo, keyState: string, keyTypeInfo: StoredTypeInfo, resultRows: number})
{

    const label = `${typeInfo.lcTypeName}ReadCapacity`

    const query = `query ReadCapacity($key: ${keyTypeInfo.gqlTypeName}!) {
        ${label}(key: $key)
    }`
