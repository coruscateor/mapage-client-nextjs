...
export default function InsertWithCapacity({typeInfo, keyState, keyTypeInfo, resultRows}: {typeInfo: StoredTypeInfo, keyState: string, keyTypeInfo: StoredTypeInfo, resultRows: number})
{

    const label = `${typeInfo.lcTypeName}InsertWithCapacity`

    const mutation = `mutation InsertNew($key: ${keyTypeInfo.gqlTypeName}!) {
        ${typeInfo.lcTypeName}InsertWithCapacity(key: $key)
    }`
    
    const paramComps = new ParamComps(label, new ParamComp("capacity", 3, JsConversionType.Number))
