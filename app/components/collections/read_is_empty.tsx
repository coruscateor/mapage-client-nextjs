...
export default function ReadIsEmpty({typeInfo, keyState, keyTypeInfo, resultRows}: {typeInfo: StoredTypeInfo, keyState: string, keyTypeInfo: StoredTypeInfo, resultRows: number})
{

    const label = `${typeInfo.lcTypeName}ReadIsEmpty`

    const query = `query ReadIsEmpty($key: ${keyTypeInfo.gqlTypeName}!) {
        ${label}(key: $key)
    }`
